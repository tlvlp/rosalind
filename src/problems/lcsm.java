...
public class lcsm {
    ...
    public static ArrayList<String> solve(ArrayList<String> inList) throws IOException {
        ...
        ArrayList<String> inListFasta = FileInputParser.parseFastaToArrayList(inList);  //parse input to a fasta compatible format
        ...
        while (!isSubstringOfAll) {                                                     //loop while finding a largest shared sequence
            longestStr = DataOperations.SelectOneLongestItem(longestList);              //return the last longest common substring (assuming that there is more than one with identical length)          
            ...
            for (int i=5; i<inListFasta.size(); i+=2) {                                 //loop through the rest of the sequences in the fasta file - assuming that there is more than two seqences
                if (!DataOperations.isValidSubStr(longestStr, inListFasta.get(i))) {    //check if the string is a substring of the current sequence
